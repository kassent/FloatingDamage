...
static void OnCrosshairRefChange(GFxMovieRoot * pRoot, GFxValue * pInfo, GFxValue * pExtraInfo, /*TESObjectREFR* pRef, */TESForm * pBaseForm, ExtraDataList * pExtraDataList, UInt32 itemCount)
{
	...
	case FormType::kFormType_MISC:
	{
		...
		{
			RelocAddr<bool(*)(void *, BGSComponent*)> IsTaggedForSearch = 0x1270170;			/// 48 89 5C 24 ? 57 48 83 EC 20 48 8B D9 48 8D 4C 24 ? 45 33 C0 48 8B FA E8 ? ? ? ? 4C 8B 83 ? ? ? ? 33 C9
			RelocPtr<void*>	pTaggedComponents = 0x5A97CE0;										/// 48 8B 0D ? ? ? ? 4C 8D 43 38
	...
	case FormType::kFormType_ALCH:
	{
		...
		RelocPtr<BGSKeyword*>	pSyringerAmmoKW = 0x591B350;///V1.10.26 ObjectTypeSyringerAmmo [KYWD:0018506F] 48 8B 15 ? ? ? ? 49 8D 8D ? ? ? ? 4C 89 6C 24 ?
		if (!pAlchemyItem->keywordForm.keywordBase.HasKeyword(*pSyringerAmmoKW))
		{
			...
			for (size_t i = 0; i < pAlchemyItem->effectItems.count; ++i)
			{
				...
				{
					RelocAddr<void(*)(MagicItem*, MagicItem::EffectItem*, float& magnitude, float& duration)> CalcMagicEffect = 0xBC46C0;///V1.10 CalaPerkEffect in this funtion.BC2D70  //48 85 C9 0F 84 ? ? ? ? 48 89 5C 24 ? 55 56
					float magnitude = 0.0f, duration = 0.0f;
					CalcMagicEffect(pAlchemyItem, pEffectItem, magnitude, duration);
					RelocAddr<float(*)(MagicItem*, MagicItem::EffectItem*)> GetRestoredHPPercent = 0x12C0F90; ///V1.10.26 //48 8B C4 53 48 83 EC 40 0F 29 70 E8
					...
					RelocAddr<void(*)(MagicItem::EffectItem*, MagicItem::EffectItem::Description&)> GetModifiedValueName = 0x0C0C870; ///V1.10.26 C0AE60 //48 89 6C 24 ? 48 89 74 24 ? 57 48 83 EC 20 48 8B F2 48 8B E9 48 8D 15 ? ? ? ?
					...
					if (modifiedValueName.pDescription && modifiedValueName.pDescription[0] && magnitude != 0.0f)
					{
						...
						RelocAddr<ActorValueInfo**(*)()> GetActorValueCollector = 0x6B1F0; //E8 ? ? ? ? 4C 8B 46 58 48 8D 4E 58 48 8B 90 ? ? ? ? 41 FF 50 08 0F 57 C9
		...
		{
			BSString str;
			auto * pDescription = &pAlchemyItem->description;
	...
	case FormType::kFormType_AMMO:
	{
		...
		if (pAmmo->keywordForm.keywordBase.HasKeyword(pKW))
		{
			...
			if (currentCharge <= 0.0f)
				currentCharge = 1.0f;
			SetGFxMember(charge, "$charge", "%d/%d", static_cast<UInt32>(currentCharge * (*RelocAddr<float*>(0x3761848))), static_cast<UInt32>(*RelocAddr<float*>(0x3761848)));
			//F3 0F 10 35 ? ? ? ? E8 ? ? ? ? 4C 8B 46 58 48 8B 90 ? ? ? ?
		}
	...
	case FormType::kFormType_WEAP:
	{
		...
		RelocAddr<void(*)(TESForm *&, InventoryItemStack &, SimpleCollector<DamageInfo>&)> CollectDamageInfo = 0xC0A6D0; ///V1.10.26 //48 8B C4 48 89 50 10 55 56 41 54
		...
		if (pWeapon->weapData.weaponType == TESObjectWEAP::InstanceData::kWeaponType_Gun)  //weapon type
		{
			...
			RelocAddr<float(*)(ObjectInstanceData&)> GetWeaponRange = 0x0661C50;///V1.10.26  40 53 48 83 EC 20 48 83 39 00 0F 57 C9
			float range = GetWeaponRange(objInst);
			SetGFxMember(range, "$rng", "%d", static_cast<UInt32>(range));
			RelocAddr<float(*)(ObjectInstanceData&, Actor*)> GetWeaponAccuracy = 0x0661D30; ///40 53 48 83 EC 30 0F 29 74 24 ? 48 8B D9 E8 ? ? ? ? 48 8B 4B 08
			try
			{
		...
		else if (pWeapon->weapData.weaponType <= TESObjectWEAP::InstanceData::kWeaponType_TwoHandedAxe)
		{
			...
			RelocAddr<const char * (*)(UInt32)>	GetWeaponSpeedDesc = 0x34D350; ///V1.10.26 //E8 ? ? ? ? 48 8D 8D ? ? ? ? 48 8B D0 E8 ? ? ? ? 48 8D 15 ? ? ? ? 4D 8B CC
	...
	case FormType::kFormType_ARMO:
	{
		...
		RelocAddr<void(*)(InventoryItem&, InventoryItemStack &, SimpleCollector<ResistInfo>&, float)> CollectResistInfo = 0xC0AB30; ///V1.10.26 //48 8B C4 F3 0F 11 58 ? 48 89 48 08 55 41 54
...
public:
	...
	{
		g_branchTrampoline.Write5Call(RelocAddr<uintptr_t>(0xAB1730), GetFnAddr(&UpdateElements_Hook)); //E8 ? ? ? ? EB 05 E8 ? ? ? ? 48 8D 9F ? ? ? ?
		g_branchTrampoline.Write5Call(RelocAddr<uintptr_t>(0xAB1737), GetFnAddr(&ClearElements_Hook)); //E8 ? ? ? ? 48 8D 9F ? ? ? ? 33 D2 48 8B CB E8 ? ? ? ? 40 88 74 24 ?
	}
...
public:
		...
		if (this->unk7D0)
		{
			...
			if (this->isDirty && this->selectedIndex >= 0 && this->dataLen && (*g_HUDDataModel) != nullptr)
			{
				...
				if (pRef != nullptr)
				{
					...
					bool tlsState[2];
					RelocAddr<void(*)(bool[], bool, bool)>  IncTLSRef = 0xEC1B90; //4C 8B C9 88 11
						...
						&& !pQC->unk228 && pQC->selectedIndex < pQC->dataLen)
					{
						...
						RelocAddr<TESForm*(*)(uintptr_t, HUDQuickContainerDataModel::Entry*)>	GetSelectedForm = 0x1A3740; //E8 ? ? ? ? 48 8B F8 48 85 C0 0F 84 ? ? ? ? 80 78 1A 32
						auto pForm = GetSelectedForm((*RelocPtr<uintptr_t>(0x590CA00)), pEntry);///////    £¿£¿£¿£¿£¿£¿£¿£¿£¿£¿£¿£¿£¿£¿£¿£¿
						if (pForm != nullptr)
						{
							RelocAddr<BGSInventoryItem *(*)(uintptr_t, HUDQuickContainerDataModel::Entry*)>	GetSelectedInventoryItem = 0x01A3650; //40 53 56 57 48 83 EC 20 33 FF
							auto pItem = GetSelectedInventoryItem((*RelocPtr<uintptr_t>(0x590CA00)), pEntry);//////
							...
							HUDMenu * pHUD = nullptr;
							if ((*g_ui) != nullptr && (pHUD = static_cast<HUDMenu*>((*g_ui)->GetMenu(&menuName)), pHUD))//crahs here?
							{
								GFxMovieRoot * movieRoot = pHUD->movie->movieRoot;
					...
					pRef->handleRefObject.DecRefHandle();
					RelocAddr<void(*)(bool[])>  DecTLSRef = 0xEC1BE0; //80 79 01 00 44 8B 05 ? ? ? ?
					DecTLSRef(tlsState);
				}
			}
			else
	...
	static void InitHook()
	{
		fnUpdateElement = HookUtil::SafeWrite64(RelocAddr<uintptr_t>(0x2D41588) + 4 * 0x8, &UpdateElement_Hook);//1.10.40 //48 8D 05 ? ? ? ? 4C 8D 05 ? ? ? ? 48 89 06 48 8D 05 ? ? ? ? 48 8D 55 E0 48 8D 4E 08 48 89 46 50
	}
};
...
public:
	...
	static void InitHook()
	{
		fnOnButtonEvent = HookUtil::SafeWrite64(RelocAddr<uintptr_t>(0x2D41AE8) + 0x8 * 8, &OnButtonEvent_Hook); //48 8D 05 ? ? ? ? 48 8B D9 C6 41 08 01 48 89 01
	}
};
			...
			if ((*g_HUDDataModel) != nullptr)
			{
				...
				if (pRef != nullptr)
				{
						...
						&& !pQC->unk228 && pQC->selectedIndex < pQC->dataLen)
					{
						...
						RelocAddr<TESForm*(*)(uintptr_t, HUDQuickContainerDataModel::Entry*)>	GetSelectedForm = 0x1A3740; //E8 ? ? ? ? 48 8B F8 48 85 C0 0F 84 ? ? ? ? 80 78 1A 32
						pForm = GetSelectedForm((*RelocPtr<uintptr_t>(0x590CA00)), pEntry);///////
						if (pForm != nullptr)
						{
							RelocAddr<BGSInventoryItem *(*)(uintptr_t, HUDQuickContainerDataModel::Entry*)>	GetSelectedInventoryItem = 0x1A3650; //40 53 56 57 48 83 EC 20 33 FF
							auto pItem = GetSelectedInventoryItem((*RelocPtr<uintptr_t>(0x590CA00)), pEntry);//////
...
public:
	...
	static void Register()
	{
		//48 89 5C 24 ? 57 48 81 EC ? ? ? ? 48 8D 59 48
		RelocPtr<BSTEventDispatcher<ViewCasterUpdateEvent> *> pDispatcher = 0x5A977A8;///V1.10.26 //48 8B 05 ? ? ? ? C6 44 24 ? ? 48 85 C0 75 1E 48 8B 15 ? ? ? ? 48 8D 0D ? ? ? ? 48 83 C2 10 E8 ? ? ? ? 48 89 05 ? ? ? ? 48 8D 54 24 ? EB 37
		...
		{
			RelocAddr<BSTEventDispatcher<ViewCasterUpdateEvent> *(*)(uintptr_t, uintptr_t)> GetCrosshairEventDispatcher = 0x9E3240;		///V1.10.26
			auto pDispatcher = GetCrosshairEventDispatcher(RelocAddr<uintptr_t>(0x5A97820).GetUIntPtr(), (*RelocAddr<uintptr_t*>(0x5906C98)) + 0x10); ///V1.10.26 TESGlobalEventSource
			if (pDispatcher != nullptr)
			{
				reinterpret_cast<BSTEventDispatcher<ViewCasterUpdateEvent> *>((uintptr_t)pDispatcher + 0x10)->AddEventSink(pCrosshairHandler);
			}
...
public:
	...
	{
		fnCreateFunction = HookUtil::SafeWrite64(RelocAddr<uintptr_t>(0x2EDB000).GetUIntPtr() + 8 * 0x30, CreateFunction_Hook); //V1.10.40   48 85 C0 0F 84 37 01 00 00 48 8B 59 48 48 85 DB 0F 84 2A 01 00 00
	}
};
...
void MessageCallback(F4SEMessagingInterface::Message * msg)
{
	if (msg->type == F4SEMessagingInterface::kMessage_GameDataReady)
	{
		...
		if (pTranslator != nullptr) 
		{
			...
			_MESSAGE("load translation file...");
		}
