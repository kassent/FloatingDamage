...
const ModInfo* DataHandler::GetLocatedModInfo(TESForm * form)
{
	if (!form || form->formID >> 24 == 0xFF)	return nullptr;
	UInt32	modIndex = form->formID >> 24;
	if (modIndex == 0xFE)
	{
		UInt16 lightModIndex = static_cast<UInt16>((form->formID & 0x00FFF000) >> 12);
		auto pModInfo = (modList.lightMods.count > lightModIndex) ? modList.lightMods[lightModIndex] : nullptr;
		if (pModInfo != nullptr)
			return pModInfo;
		return (modList.loadedMods.count > 0xFE) ? modList.loadedMods[0xFE] : nullptr;
	}
	else
	{
		return (modList.loadedMods.count > modIndex) ? modList.loadedMods[modIndex] : nullptr;
	}
}

const ModInfo* DataHandler::GetLastChangeModInfo(TESForm * form)
{
	if (!form)	return nullptr;
	return form->GetLastModifiedMod();
}