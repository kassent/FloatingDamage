...
public:
	...
	static uintptr_t GetEffectiveAddress(uintptr_t RVAScan) {
		return RelocationManager::s_baseAddr + RVAScan;
	}
...
public:
	...
	// Address only
	RVAScan(uintptr_t RVAScan) {
		AddressMap addr = { { CURRENT_RELEASE_RUNTIME, RVAScan } };
		init(addr, NULL, 0);
	}

	// Address + sig
	RVAScan(uintptr_t RVAScan, const char* sig, int offset = 0, int indirectOffset = 0, int instructionLength = 0) {
		AddressMap addr = { { CURRENT_RELEASE_RUNTIME, RVAScan } };
		init(addr, sig, offset, indirectOffset, instructionLength);
	}
	...
	void Set(uintptr_t RVAScan) {
		data->effectiveAddress = RVAManager::GetEffectiveAddress(RVAScan);
	}
